...
def path_scanning(graph, cost_matrix, t_rule):
    ...
    while True:
        ...
        while True:
            ...
            tmp_ratio = 1
            if t_r < 0.05:
                tmp_ratio = random.uniform(0.7, 0.85)
            if t_r < 0.12:
                tmp_ratio = random.uniform(0.8, 1)
            elif t_r < 0.24:
                tmp_ratio = random.uniform(0.85, 1)
            elif t_r < 0.55:
                tmp_ratio = random.uniform(0.9, 1)
            elif t_r < 0.72:
                tmp_ratio = random.uniform(0.95, 1)
            else:
                tmp_ratio = 1
            for edge in gra:
                if loads[count] + edge[3] <= capacity * tmp_ratio:
                    ...
                    if d_se < tmp_cost:
                        ...
                        tmp_edge = edge
            if tmp_edge:
                t_se = shortest_path(cost_matrix, strat, tmp_edge[0])
                t_s1 = shortest_path(cost_matrix, strat, depot)
                t_s2 = shortest_path(cost_matrix, depot, tmp_edge[0])
                if strat != depot and t_se == t_s1 + t_s2:
                    break
                [x, y, z, w] = tmp_edge
...
def solve_CARP(seed):
    ...
    for line in data:
        if line != "END":
            ...
            if d[0] in graph:
                graph[d[0]][d[1]] = (d[2], d[3])
            else:
                graph[d[0]] = {d[1]: (d[2], d[3])}
            if d[1] in graph:
                graph[d[1]][d[0]] = (d[2], d[3])
            else:
                graph[d[1]] = {d[0]: (d[2], d[3])}
...
if __name__ == '__main__':
    ...
    Task = [seed + i + random.randint(60, 300) for i in range(0, 24, 3)]  # 生成16个随机任务， 每个任务是2个整数， 需要计算两数之和与积
    # print('Task', Task)
